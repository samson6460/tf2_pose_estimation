    ...
    def labelme_json_to_dataset(
        ...
        Args:
            ...
            num_stacks: An integer,
                number of stacks for hourglass network.
...
class KeypointSequence(Sequence):
    ...
    Details are in `Keypoint_reader.labelme_json_to_sequence()`.
    """
...
def decode(label, zoom_r, method="mean"):
    ...
    Args:
        label: An array,
            shape: (heatmap height, heatmap width, num_points).
...
def draw_img_ann(img, label,
                 ...
                 point_radius=8,
                 linewidth=4):
    """Draw image with its annotation by OpenCV.
    ...
    for class_i in range(class_num):
        ...
        img = cv2.circle(img, (x_val, y_val),
...
def get_class_weight(label_data, method="alpha"):
    ...
    Args:
        label_data: A ndarray of shape(batch size, rows, cols, channels).
        method: A string,
...
def draw_heatmap(height, width,
    ...
    Args:
        ...
        sigma: An integer,
            standard deviation of 2D gaussian distribution.
        mode: A string, one of "gs" or "exp".
            "gs": Gaussian heatmap.
            "exp": Exponential heatmap(proposed by 
            https://pubmed.ncbi.nlm.nih.gov/31683913/).

    Returns:
        Heatmap array with shape (height, width).
    """
...
def draw_heatmap_batch(size, point, sigma, mode="gs"):
    """Generate multiple heatmaps by points.

    Args:
        size: (heatmap height, heatmap width).
        point: An array, shape: (batches, num_points, 2).
        sigma: An integer,
            standard deviation of 2D gaussian distribution.
        mode: A string, one of "gs" or "exp".
            "gs": Gaussian heatmap.
    ...
    Returns:
        An array of batched heatmaps with shape (batch size, height, width).
    """
