...
WEIGHTS_PATH_X = ("https://github.com/bonlime/keras-deeplab-v3-plus"
                  "/releases/download/1.1"
                  "/deeplabv3_xception_tf_dim_ordering_tf_kernels.h5")
WEIGHTS_PATH_MOBILE = ("https://github.com/bonlime/keras-deeplab-v3-plus"
                       "/releases/download/1.1"
                       "/deeplabv3_mobilenetv2_tf_dim_ordering_tf_kernels.h5")
WEIGHTS_PATH_X_CS = ("https://github.com/bonlime/keras-deeplab-v3-plus"
                     "/releases/download/1.2"
                     "/deeplabv3_xception_tf_dim_ordering_"
                     "tf_kernels_cityscapes.h5")
WEIGHTS_PATH_MOBILE_CS = ("https://github.com/bonlime/keras-deeplab-v3-plus"
                          "/releases/download/1.2"
                          "/deeplabv3_mobilenetv2_tf_dim_ordering_"
                          "tf_kernels_cityscapes.h5")
...
def SepConv_BN(x, filters, prefix,
    ...
    Args:
        ...
        depth_activation: flag to use activation 
        between depthwise & poinwise convs
...
def _conv2d_same(x, filters, prefix,
                 stride=1, kernel_size=3,
                 rate=1):
    """Implements right 'same' padding for even kernel sizes.
...
def _xception_block(inputs, depth_list, prefix,
                    skip_connection_type, stride,
                    rate=1, depth_activation=False,
                    return_skip=False):
    ...
    Args:
        ...
        return_skip: flag to return additional tensor
            after 2 SepConvs for decoder
    """
    residual = inputs
    for i in range(3):
...
def deeplabv3(pretrained_weights='pascal_voc',
    ...
    if backbone == 'xception':
        ...
        x = _conv2d_same(x, 64, 'entry_flow_conv1_2',
        ...
        for i in range(16):
            x = _xception_block(x, [728, 728, 728],
                                ...
                                stride=1, rate=middle_block_rate,
                                depth_activation=False)

        x = _xception_block(x, [728, 1024, 1024], 'exit_flow_block1',
                            skip_connection_type='conv',
                            stride=1, rate=exit_block_rates[0],
                            depth_activation=False)
        x = _xception_block(x, [1536, 1536, 2048], 'exit_flow_block2',
                            skip_connection_type='none',
                            stride=1, rate=exit_block_rates[1],
    ...
    b4 = BatchNormalization(name='image_pooling_BN',
                            epsilon=1e-5)(b4)
        ...
        if pretrained_weights == 'pascal_voc':
            if backbone == 'xception':
                ...
                weights_path = get_file(
                    ...
                    WEIGHTS_PATH_MOBILE,
                    cache_subdir='models')
        elif pretrained_weights == 'cityscapes':
            if backbone == 'xception':
                weights_path = get_file(
                    ('deeplabv3_xception_tf_dim_'
                     'ordering_tf_kernels_cityscapes.h5'),
                    WEIGHTS_PATH_X_CS,
                    cache_subdir='models')
            else:
                weights_path = get_file(
                    ('deeplabv3_mobilenetv2_tf_dim_'
                     'ordering_tf_kernels_cityscapes.h5'),
